...
var edgeLength = function edgeLength(initialGraph, updatedGraph) {
  ...
  var r = function r(e) {
    ...
    var initLength = agoraGraph.norm(initialSorted[e.source], initialSorted[e.target]);
    var uLength = agoraGraph.norm(updatedSorted[e.source], updatedSorted[e.target]);
  ...
  if (withDelaunay) {
    var delaunayEdges = agoraGraph.delaunay(initialNodes);
    return {
      value: agoraGraph.round(delta(delaunayEdges, r), -6)
    };
  ...
  return {
    value: agoraGraph.round(delta(initialGraph.edges, r), -6)
  };
};
...
function calculateEdgeRatio(graph) {
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      var norm_uv = agoraGraph.norm(u, v);
...
var aspectRatio = function aspectRatio(initialGraph, updatedGraph) {
  var initialNodes = initialGraph.nodes;
  var updatedNodes = updatedGraph.nodes;
  var w = agoraGraph.right(agoraGraph.right(initialNodes)) - agoraGraph.left(agoraGraph.left(initialNodes));
  var h = agoraGraph.bottom(agoraGraph.bottom(initialNodes)) - agoraGraph.top(agoraGraph.top(initialNodes));
  var wp = agoraGraph.right(agoraGraph.right(updatedNodes)) - agoraGraph.left(agoraGraph.left(updatedNodes));
  var hp = agoraGraph.bottom(agoraGraph.bottom(updatedNodes)) - agoraGraph.top(agoraGraph.top(updatedNodes));
  return {
...
var improvedAspectRatio = function improvedAspectRatio(initial, updated) {
  var initialSize = [agoraGraph.round(agoraGraph.bottom(agoraGraph.bottom(initial.nodes)) - agoraGraph.top(agoraGraph.top(initial.nodes))), agoraGraph.round(agoraGraph.right(agoraGraph.right(initial.nodes)) - agoraGraph.left(agoraGraph.left(initial.nodes)))],
      updatedSize = [agoraGraph.round(agoraGraph.bottom(agoraGraph.bottom(updated.nodes)) - agoraGraph.top(agoraGraph.top(updated.nodes))), agoraGraph.round(agoraGraph.right(agoraGraph.right(updated.nodes)) - agoraGraph.left(agoraGraph.left(updated.nodes)))];
  return {
...
function calculateConvexHullDistances(hull) {
  ...
  for (var angle = 0; angle < 360; angle += 10) {
    rays.push({
      angle: angle,
      length: 1,
      theta: agoraGraph.round(angle * (Math.PI / 180), -14)
    });
  ...
  _.forEach(rays, function (ray) {
    ...
    var cartesianRay = agoraGraph.toCartesian(ray);
    ...
    if (intersection) distances.push(agoraGraph.magnitude(intersection));else throw Error('it is supposed to intersect :(');
  });
...
function convertNodes(nodes) {
  ...
  return _.flatMap(nodes, function (n) {
    var t = agoraGraph.top(n),
        l = agoraGraph.left(n),
        r = agoraGraph.right(n),
        b = agoraGraph.bottom(n);
...
var costFunction = function costFunction(initialGraph, updatedGraph) {
  ...
  for (var u_index = 0; u_index < initialNodes.length; u_index++) {
    var u = initialNodes[u_index];
    var u_prime = updatedNodes[u_index];
    sum += agoraGraph.normX(u_prime, u) + agoraGraph.normY(u_prime, u);
  }
...
var phiChange = function phiChange(initialGraph, updatedGraph) {
  ...
  for (var u_index = 0; u_index < initialNodes.length; u_index++) {
    var u = initialNodes[u_index];
    var u_prime = updatedNodes[u_index];
    sum += Math.pow(agoraGraph.normX(u_prime, u), 2) + Math.pow(agoraGraph.normY(u_prime, u), 2);
  }
...
function scaleChange(initial, updated) {
  ...
  return ___default.reduce(initialCenteredNodes, function (_ref, _ref2) {
    ...
    var x = _ref2.x,
        ...
        index = _ref2.index;
    var projected = {
      x: agoraGraph.round(x * scale.x, -9),
      y: agoraGraph.round(y * scale.y, -9)
    };
    ...
    var diff = agoraGraph.norm(projected, up);
...
var euclideanDistance = function euclideanDistance(initialGraph, updatedGraph) {
  ...
  for (var index = 0; index < nodesLength; index++) {
    ...
    var diff = agoraGraph.norm(node, upNode);
    change += diff;
...
function iv(u, v, u_prime, v_prime) {
  return agoraGraph.left(u) > agoraGraph.left(v) && agoraGraph.left(u_prime) < agoraGraph.left(v_prime) || agoraGraph.left(u) < agoraGraph.left(v) && agoraGraph.left(u_prime) > agoraGraph.left(v_prime) || agoraGraph.top(u) > agoraGraph.top(v) && agoraGraph.top(u_prime) < agoraGraph.top(v_prime) || agoraGraph.top(u) < agoraGraph.top(v) && agoraGraph.top(u_prime) > agoraGraph.top(v_prime);
}
...
var l1MetricLength = function l1MetricLength(initial, updated) {
  var initialResult = Math.max(agoraGraph.round(agoraGraph.bottom(agoraGraph.bottom(initial.nodes)) - agoraGraph.top(agoraGraph.top(initial.nodes))), agoraGraph.round(agoraGraph.right(agoraGraph.right(initial.nodes)) - agoraGraph.left(agoraGraph.left(initial.nodes))));
  var updatedResult = Math.max(agoraGraph.round(agoraGraph.bottom(agoraGraph.bottom(updated.nodes)) - agoraGraph.top(agoraGraph.top(updated.nodes))), agoraGraph.round(agoraGraph.right(agoraGraph.right(updated.nodes)) - agoraGraph.left(agoraGraph.left(updated.nodes))));
  return {
...
var sizeIncrease = function sizeIncrease(initial, updated) {
  var initialHull = d3.polygonHull(convertNodes$1(initial.nodes));
  var updatedHull = d3.polygonHull(convertNodes$1(updated.nodes));
  if (initialHull === null || updatedHull === null) return {
    value: -1,
    error: 'could not compute initial or updated convex hull'
  };
  var initialArea = d3.polygonArea(initialHull);
  var updatedArea = d3.polygonArea(updatedHull);
  return {
...
exports.EdgeBased = EdgeBased;
exports.GlobalShape = GlobalShape;
exports.NodeMovement = NodeMovement;
exports.OrthogonalOrdering = OrthogonalOrdering;
